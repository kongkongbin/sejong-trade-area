...
export default function PopulationTab({ data, loading, workplaceData }) {
  if (loading) {
  ...
  return (
    <div>
      ...
      {/* 직장인구 + 배후세대 */}
      {workplaceData?.matched && (
        <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid #f0f1f3' }}>
          <p style={{ fontSize: 13, fontWeight: 600, margin: '0 0 12px', color: '#1a2233' }}>
            직장인구 · 배후세대
          </p>
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1, background: '#f4f5f7', borderRadius: 8, padding: '10px 12px' }}>
              <p style={{ fontSize: 11, color: '#888', margin: '0 0 4px' }}>직장인구</p>
              <p style={{ fontSize: 18, fontWeight: 600, margin: 0 }}>
                {workplaceData.totWorker.toLocaleString()}명
              </p>
              <p style={{ fontSize: 11, color: '#aaa', margin: '4px 0 0' }}>
                사업체 {workplaceData.corpCnt.toLocaleString()}개
              </p>
            </div>
            <div style={{ flex: 1, background: '#f4f5f7', borderRadius: 8, padding: '10px 12px' }}>
              <p style={{ fontSize: 11, color: '#888', margin: '0 0 4px' }}>배후세대</p>
              <p style={{ fontSize: 18, fontWeight: 600, margin: 0 }}>
                {workplaceData.householdCnt.toLocaleString()}세대
              </p>
              <p style={{ fontSize: 11, color: '#aaa', margin: '4px 0 0' }}>
                가구원 평균 {workplaceData.avgFamilyMemberCnt.toFixed(1)}명
              </p>
            </div>
          </div>
        </div>
